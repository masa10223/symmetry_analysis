...
## Detect Area
Moving the scale bar labeled "Scroll Threshold" will binarize the area displayed in the right window according to its value. The scale bar can be moved freely using the numeric keypad. You can also binarize the image by typing a number between 0 and 255 in the text input form above it. In the latter case, the image will be displayed in the right window when you type Detect Area.
